...
def run_tests_transductive(n_runs, include_greedy=True, include_e2e=True, save_results=True, custom_training_dimensions=False):
    ...
    for td in training_dimensions:
        ...
        if save_results:
            if include_e2e:
                with open('./e2e/results_transductive(3layers)_{}runs'.format(n_runs), 'wb') as output:
                    pickle.dump(results_e2e, output)
            if include_greedy:
                with open('./greedy/results_transductive(3layers)_{}runs'.format(n_runs), 'wb') as output:
