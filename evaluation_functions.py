...
def plot_means_and_intervals(history_all, title, barwidth=0.1, confidence_level=0.95):
    means_kenn_list = []
    stds_kenn_list = []
    ...
    r1 = np.arange(len(means)) * 4.5 * barwidth
    r_list = []

    r_list.append([x + barwidth for x in r1])
    r_list.append([x + 2 * barwidth for x in r1])
    r_list.append([x + 3 * barwidth for x in r1])

    # Make the plot
    plt.bar(r1, means, width=barwidth,
            edgecolor='white', label='NN')

    for n_layers, means_kenn in zip([1,2,3], means_kenn_list):
        plt.bar(r_list[n_layers - 1], means_kenn, width=barwidth,
                edgecolor='white', label='KENN l={}'.format(n_layers))
    ...
    plt.errorbar(
        ...
        elinewidth=2,
        ls='none')
    for r, means_kenn, confidence_margins_kenn in zip(r_list, means_kenn_list, confidence_margins_kenn_list):
        plt.errorbar(
            r,
            means_kenn,
    ...
    plt.xticks([r + 1.5 * barwidth for r in r1], history.keys())
    plt.legend(loc='best')
    plt.title(title)
...
def plot_accuracies_changing_n_layers(history_all, title, confidence_level=0.95):
    ...
    plt.figure(figsize=(9, 5))
    r = np.array([0.1, 0.25, 0.5, 0.75, 0.9])

    # Make the plot
    plt.plot(r, means, label='NN')

    for n_layers, means_kenn in zip(range(n_layers_tried), means_kenn_list):
        plt.plot(r, means_kenn, label='KENN l={}'.format(n_layers + 1))
...
def plot_deltas(history_all, barwidth=0.3, title='', other_deltas='', confidence_level=0.95):
    mean_deltas_list = []
    stds_deltas_list = []
    confidence_margins_list = []
    for history in history_all:
        ...
        mean_deltas_list.append(means_deltas)
        stds_deltas_list.append(stds_deltas)
        confidence_margins_list.append(confidence_margins_delta)
    ...
    r1 = np.arange(len(deltas_RNM_i)) * 5.5 * barwidth
    r_list = []
    ...
    for n_layers, r, means_deltas, confidence_margins_delta in zip([1,2,3], r_list, mean_deltas_list, confidence_margins_list):
        plt.bar(r, means_deltas, width=barwidth, edgecolor='white', label='delta KENN l={}'.format(n_layers))
        plt.errorbar(
            r,
            means_deltas,
    ...
    if other_deltas == 'i':
        ...
        plt.bar(r2, deltas_SBR_i, width=barwidth,
                edgecolor='white', label='delta SBR')
        plt.bar(r3, deltas_RNM_i, width=barwidth,
                edgecolor='white', label='delta RNM')

    elif other_deltas == 't':
        r2 = [x + barwidth for x in r]
        r3 = [x + barwidth for x in r2]
        plt.bar(r2, deltas_SBR_t, width=barwidth, edgecolor='white', label='delta SBR')
        plt.bar(r3, deltas_RNM_t, width=barwidth, edgecolor='white', label='delta RNM')
...
def plot_histograms_all(history_all, title, bw=0.5, bins=20):
    n_runs = len(history_all[0][list(history_all[0].keys())[0]]['NN'])

    fig, axes = plt.subplots(len(history_all[0].keys()), 2, figsize=(12, 13))
    colors = ['red', 'green', 'gray']

    for n_layers, color, history in zip([1, 2, 3], colors, history_all):
        for i, num in enumerate(history.keys()):
            ...
            axes[i, 0].hist(test_accuracies_kenn, density=True,
                            bins=b, alpha=0.3, label='KENN l={}'.format(n_layers), color=color)
            sns.kdeplot(test_accuracies_kenn,
                        ax=axes[i, 0], bw_adjust=bw, color=color)

            axes[i, 0].set_title("Training dimension: {}".format(num))
            axes[i, 0].legend(loc='best')

            axes[i, 1].hist(deltas, density=True, bins=bins,
                            alpha=0.3, label='KENN l={}'.format(n_layers), color=color)
            sns.kdeplot(deltas, ax=axes[i, 1], bw_adjust=bw, color=color)
            axes[i, 1].set_title("Deltas for training dimension {}%".format(num))
...
def print_stats(history_all):
    n_layers_tried = len(history_all)
    for n_layers, history in zip(range(start=1, stop=n_layers_tried), history_all):
        print("============ {}% ============".format(n_layers))
