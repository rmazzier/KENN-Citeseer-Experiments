...
def train_and_evaluate_kenn_inductive_greedy(percentage_of_training, verbose=True):
    ...
    nn_train_preactivations, nn_valid_preactivations, nn_test_preactivations, nn_results = train_and_evaluate_standard(
    ...
    for epoch in range(s.EPOCHS_KENN):
        ...
        t_predictions = kenn_model(
            [nn_train_preactivations, relations_inductive_training, index_x_train, index_y_train])
    ...
    predictions_test = kenn_model(
        [nn_test_preactivations, relations_inductive_test, index_x_test, index_y_test])
...
if __name__ == "__main__":
    ...
    nn_history, greedy_kenn_history = train_and_evaluate_kenn_inductive_greedy(
        0.75)
